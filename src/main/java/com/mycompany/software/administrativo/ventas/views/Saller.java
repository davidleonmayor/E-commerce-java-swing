...
// Crea un modelo de tabla y añade las IDs de las facturas
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        // 1. catch inputs user
        int document = Integer.parseInt(documentInput.getText());
        int password = Integer.parseInt(passwordInput.getText());
        String name = nameInput.getText();
        String lastName = lastNameInput.getText();

        // 2. Make a new sellr with the captured user data
        SellerQuery sellerQuery;
        try {
            sellerQuery = new SellerQuery();
            sellerQuery.create(document, password, name, lastName);
        } catch (SQLException ex) {
